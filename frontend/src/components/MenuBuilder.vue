<template>
    <div class="flex flex-col h-full bg-gray-50 dark:bg-gray-900 pb-20 transition-colors duration-300">
        
        <!-- HEADER MENU BUILDER -->
        <div class="bg-scoutViolet dark:bg-gray-800 text-white pt-5 pb-6 px-6 rounded-b-[30px] shadow-lg z-30 flex-none relative no-print transition-colors">
        ...
        <!-- BARRE D'ACTIONS -->
        <div class="bg-white dark:bg-gray-800 px-4 py-4 shadow-sm z-20 flex justify-between items-center sticky top-0 -mt-4 pt-6 no-print border-b border-transparent dark:border-gray-700 transition-colors">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-5 pb-28 no-print">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 transition-colors">
                ...
                <div v-for="(recipe, index) in currentMealRecipes" :key="recipe.id || index" class="flex items-center justify-between py-2 border-b border-gray-50 dark:border-gray-700 transition-colors">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-xs text-gray-400 dark:text-gray-500 font-bold uppercase transition-colors">{{ recipe.dish_type }}</p>
                            <p class="text-sm font-medium text-gray-900 dark:text-gray-200 transition-colors">{{ recipe.name }}</p>
                        </div>
                    </div>
                    <button @click="retirerRecette(index, recipe.id)" class="text-gray-300 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 transition-colors">
                ...
                <button @click="$router.push('/recipes')" class="w-full mt-4 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-lg py-3 text-sm font-medium text-gray-500 dark:text-gray-400 hover:border-scoutViolet dark:hover:border-purple-400 hover:text-scoutViolet dark:hover:text-purple-400 transition-colors flex justify-center items-center gap-2">
                    <span class="text-lg leading-none">+</span> Ajouter un plat / Recette
                </button>
            </div>
            
            <button @click="genererBordereau" class="w-full bg-[#5b2b82] dark:bg-purple-600 hover:bg-purple-900 dark:hover:bg-purple-700 text-white font-bold py-3.5 rounded-xl shadow-md transition-all active:scale-95 text-sm flex justify-center items-center gap-2">
               Afficher la liste de courses
            </button>
        </div>

        <!-- MODALE : BORDEREAU DE COURSES -->
            ...
            <div v-if="showShoppingModal" class="fixed inset-0 z-50 flex flex-col justify-end printable-modal">
                ...
                <div class="relative bg-gray-50 dark:bg-gray-900 w-full h-[90%] rounded-t-3xl shadow-2xl flex flex-col z-10 overflow-hidden printable-content transition-colors">
                    ...
                    <div class="flex-1 overflow-y-auto p-4 pb-20">
                        ...
                        <div class="bg-[#5b2b82] dark:bg-purple-900/60 text-white rounded-xl p-4 mb-6 shadow-md flex justify-between items-center no-print transition-colors border dark:border-purple-800">
                            <div>
                        ...
                        <div v-for="(items, category) in groupedShoppingList" :key="category" class="mb-6">
                            ...
                            <div class="space-y-2">
                                <label v-for="(item, index) in items" :key="index" class="rounded-xl p-3 shadow-sm border flex items-center justify-between cursor-pointer transition-all group" :class="item.isChecked ? 'border-violet-200 dark:border-purple-800 bg-violet-50/30 dark:bg-purple-900/20 opacity-75' : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 hover:border-violet-200 dark:hover:border-purple-700'">
