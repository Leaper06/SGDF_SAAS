    ...
    <div class="flex-1 flex flex-col min-h-0 relative">
      ...
      <div class="bg-white px-4 pb-4 border-b border-gray-100 shadow-sm z-0">
        ...
        <div class="grid grid-cols-7 gap-y-4 text-center text-sm items-center">
          <div v-for="(jour, index) in joursDuCalendrier" :key="index" @click="jour.isCurrentMonth ? selectionnerDate(jour.date) : null" :class="['relative flex flex-col items-center justify-center py-1 transition-colors', jour.isCurrentMonth && jour.etatCamp === 'aucun' ? 'cursor-pointer hover:bg-gray-100 rounded-full' : '', !jour.isCurrentMonth ? 'text-gray-300' : 'font-medium', jour.etatCamp === 'debut' ? 'bg-scoutViolet text-white rounded-l-full cursor-pointer' : '', jour.etatCamp === 'fin' ? 'bg-scoutViolet text-white rounded-r-full cursor-pointer' : '', jour.etatCamp === 'milieu' ? 'bg-scoutViolet text-white cursor-pointer' : '', jour.etatCamp === 'aucun' ? 'text-gray-700' : '']">
      ...
      <main class="flex-1 overflow-y-auto p-4 pb-24 scrollbar-hide">
        ...
        <div v-for="camp in campsDuMois" :key="camp.id" class="bg-white rounded-2xl shadow-sm border-l-4 border-scoutViolet p-4 mb-4 hover:shadow-md transition-shadow">
          ...
          <div class="relative bg-white w-full rounded-t-3xl shadow-2xl flex flex-col pb-8 z-10">
            ...
            <div class="px-6 overflow-y-auto scrollbar-hide">
              ...
              <div class="flex p-1 bg-gray-100 rounded-xl mb-6">
                <button @click="newEvent.type = 'journee'" :class="['flex-1 py-2 text-sm font-bold rounded-lg transition-all', newEvent.type === 'journee' ? 'bg-white text-scoutViolet shadow-sm' : 'text-gray-400 hover:text-gray-600']">Journée</button>
                <button @click="newEvent.type = 'weekend'" :class="['flex-1 py-2 text-sm font-bold rounded-lg transition-all', newEvent.type === 'weekend' ? 'bg-white text-scoutViolet shadow-sm' : 'text-gray-400 hover:text-gray-600']">Week-end</button>
              </div>
              <div class="space-y-4">
                <div>
...
<script setup>
import { changerMois, moisActuelTexte, joursDuCalendrier, selectionnerDate, loading, campsDuMois, formatWeekday, formatDay, ouvrirPlanning, showAddModal, fermerModal, newEvent, soumettreEvenement } from '../store.js'
</script>