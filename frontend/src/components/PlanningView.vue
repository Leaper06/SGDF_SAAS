<template>
    ...
    <div class="flex-1 flex flex-col min-h-0 relative bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
      ...
      <div class="bg-scoutBlue dark:bg-gray-800 text-white pt-6 pb-4 px-4 rounded-b-3xl shadow-md z-20 flex flex-col items-center transition-colors">
      ...
      <div class="bg-white dark:bg-gray-800 px-4 py-3 flex justify-between items-center border-b border-gray-100 dark:border-gray-700 shadow-sm z-10 transition-colors">
      ...
      <main class="flex-1 overflow-y-auto pb-24 scrollbar-hide">
        ...
        <template v-for="(slots, jour) in slotsParJour" :key="jour">
          ...
          <div v-show="joursOuverts[jour] !== false" class="p-4 space-y-4">
            <div v-for="slot in slots" :key="slot.id" :class="['bg-white dark:bg-gray-800 rounded-[1.25rem] shadow-sm border-l-4 p-4 flex gap-4 transition-colors', getTheme(slot.slot_type).border]">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex justify-between items-start">
                  <h4 class="font-bold text-gray-900 dark:text-white text-[15px] pr-2 truncate transition-colors">{{ slot.title }}</h4>
                ...
                <button v-if="slot.slot_type === 'logistique'" @click.stop="ouvrirGestionTentes" class="mt-3 flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 flex flex-col gap-3 mb-6 mx-4 transition-colors">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-bold text-gray-900 dark:text-white transition-colors">Effectifs & Intendance</h4>
                    <p class="text-[11px] text-gray-500 dark:text-gray-400 transition-colors">Registre et liste de courses du week-end</p>
                </div>
            </div>
            
            <div class="grid grid-cols-2 gap-2 mt-2">
                <button @click="ouvrirGestionPresence" class="w-full text-xs font-semibold py-2.5 rounded-xl transition-colors bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200">
                    Faire l'appel
                </button>
                
                <button @click="genererBordereauGlobal" class="w-full text-xs font-semibold py-2.5 rounded-xl transition-colors bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-800/40 text-blue-700 dark:text-blue-400 flex items-center justify-center gap-1.5">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <button @click="ouvrirAjoutSlot" class="absolute bottom-24 right-6 w-14 h-14 bg-[#e85d22] text-white rounded-full shadow-lg shadow-orange-500/30 dark:shadow-orange-900/50 flex items-center justify-center hover:bg-orange-600 transition-transform active:scale-95 z-30">
      ...
      <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-full" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-full">
        <div v-if="showAddSlotModal" class="absolute inset-0 z-50 flex flex-col justify-end">
          ...
          <div class="relative bg-white dark:bg-gray-800 w-full rounded-t-3xl shadow-2xl flex flex-col pb-8 z-10 transition-colors">
            <div class="w-full flex justify-center py-4 cursor-pointer" @click="fermerSlotModal"><div class="w-12 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full transition-colors"></div></div>
            <div class="px-6 overflow-y-auto scrollbar-hide">
              <h2 class="text-xl font-extrabold text-gray-900 dark:text-white mb-6 transition-colors">Ajouter une activité</h2>
              <div class="space-y-5">
                <div>
                  <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 transition-colors">Type</label>
                  <div class="grid grid-cols-2 gap-2">
                    <button @click="newSlot.slot_type = 'logistique'" :class="['py-2 text-xs font-bold rounded-lg border-2 transition-all', newSlot.slot_type === 'logistique' ? 'border-gray-500 bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-white' : 'border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-500']">Logistique</button>
                    ...
                    <button @click="newSlot.slot_type = 'repas'" :class="['py-2 text-xs font-bold rounded-lg border-2 transition-all', newSlot.slot_type === 'repas' ? 'border-scoutBlue dark:border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-scoutBlue dark:text-blue-400' : 'border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-500']">Repas</button>
                    <button @click="newSlot.slot_type = 'spi'" :class="['py-2 text-xs font-bold rounded-lg border-2 transition-all', newSlot.slot_type === 'spi' ? 'border-[#009ee0] dark:border-cyan-500 bg-cyan-50 dark:bg-cyan-900/20 text-[#009ee0] dark:text-cyan-400' : 'border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-500']">Veillée / Spi</button>
                  </div>
                </div>
                <div>
      ...
      <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-full" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-full">
        <div v-if="showEditCampModal" class="absolute inset-0 z-50 flex flex-col justify-end">
          ...
          <div class="relative bg-white dark:bg-gray-800 w-full rounded-t-3xl shadow-2xl flex flex-col pb-8 z-10 transition-colors">
            <div class="w-full flex justify-center py-4 cursor-pointer" @click="fermerEditCampModal"><div class="w-12 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full transition-colors"></div></div>
            <div class="px-6 overflow-y-auto scrollbar-hide">
              <h2 class="text-xl font-extrabold text-gray-900 dark:text-white mb-6 transition-colors">Modifier le week-end</h2>
              <div class="space-y-5">
                <div>
      ...
      <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-full" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-full">
        <div v-if="showEditSlotModal" class="absolute inset-0 z-50 flex flex-col justify-end">
          ...
          <div class="relative bg-white dark:bg-gray-800 w-full rounded-t-3xl shadow-2xl flex flex-col pb-8 z-10 transition-colors">
            <div class="w-full flex justify-center py-4 cursor-pointer" @click="fermerEditSlotModal"><div class="w-12 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full transition-colors"></div></div>
            <div class="px-6 overflow-y-auto scrollbar-hide">
              <h2 class="text-xl font-extrabold text-gray-900 dark:text-white mb-6 transition-colors">Modifier l'activité</h2>
              <div class="space-y-5">
                <div>
                  <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 transition-colors">Type</label>
                  <div class="grid grid-cols-2 gap-2">
                    <button @click="editSlot.slot_type = 'logistique'" :class="['py-2 text-xs font-bold rounded-lg border-2 transition-all', editSlot.slot_type === 'logistique' ? 'border-gray-500 bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-white' : 'border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-500']">Logistique</button>
                    ...
                    <button @click="editSlot.slot_type = 'repas'" :class="['py-2 text-xs font-bold rounded-lg border-2 transition-all', editSlot.slot_type === 'repas' ? 'border-scoutBlue bg-blue-50 dark:bg-blue-900/20 text-scoutBlue dark:text-blue-400' : 'border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-500']">Repas</button>
                    <button @click="editSlot.slot_type = 'spi'" :class="['py-2 text-xs font-bold rounded-lg border-2 transition-all', editSlot.slot_type === 'spi' ? 'border-[#009ee0] bg-cyan-50 dark:bg-cyan-900/20 text-[#009ee0] dark:text-cyan-400' : 'border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-500']">Veillée / Spi</button>
                  </div>
                </div>
                <div>
                  <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1 transition-colors">Titre</label>
    ...
    <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-full" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-full">
        <div v-if="showTentsModal" class="fixed inset-0 z-50 flex flex-col justify-end">
            ...
            <div class="relative bg-white dark:bg-gray-800 w-full h-[85vh] rounded-t-3xl shadow-2xl flex flex-col z-10 overflow-hidden transition-colors">
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50 transition-colors">
                    <div>
                        <h3 class="font-extrabold text-lg text-gray-900 dark:text-white transition-colors">Tentes du week-end</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400 font-medium transition-colors">Sélectionne le matériel nécessaire</p>
                    </div>
                    <button @click="showTentsModal = false" class="p-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 rounded-full text-gray-600 dark:text-gray-300 transition-colors">
    ...
    <div v-if="showAttendanceModal" class="fixed inset-0 z-50 flex flex-col justify-end">
        ...
        <div class="relative bg-white dark:bg-gray-800 w-full h-[85vh] rounded-t-3xl shadow-2xl flex flex-col z-10 overflow-hidden transition-colors">
            ...
            <div class="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/30 dark:bg-gray-900/30 transition-colors">
                ...
                <div>
                    <h4 class="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3 transition-colors">La Maîtrise ({{ chefs.length }})</h4>
                    <div class="space-y-2">
                        <div v-for="chef in chefs" :key="chef.id" @click="togglePresence(chef.id)" 
                             :class="['border rounded-2xl p-3 flex justify-between items-center cursor-pointer transition-all', selectedAdherents.includes(String(chef.id)) ? 'border-blue-500 dark:border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-600']">
                            <span class="font-bold text-gray-900 dark:text-white text-sm transition-colors">{{ chef.prenom }} {{ chef.nom }}</span>
                            <div :class="['w-5 h-5 rounded border-2 flex items-center justify-center transition-colors', selectedAdherents.includes(String(chef.id)) ? 'bg-blue-500 border-blue-500 text-white' : 'border-gray-300 dark:border-gray-600']">
                ...
                <div>
                    <h4 class="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3 transition-colors">Les Jeunes ({{ jeunes.length }})</h4>
                    <div class="space-y-2">
                        <div v-for="jeune in jeunes" :key="jeune.id" @click="togglePresence(jeune.id)" 
                             :class="['border rounded-2xl p-3 flex justify-between items-center cursor-pointer transition-all', selectedAdherents.includes(String(jeune.id)) ? 'border-blue-500 dark:border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-600']">
                            <span class="font-bold text-gray-900 dark:text-white text-sm transition-colors">{{ jeune.prenom }} {{ jeune.nom }}</span>
                            <div :class="['w-5 h-5 rounded border-2 flex items-center justify-center transition-colors', selectedAdherents.includes(String(jeune.id)) ? 'bg-blue-500 border-blue-500 text-white' : 'border-gray-300 dark:border-gray-600']">
            ...
            <div class="p-6 border-t border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] transition-colors">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="text-xl font-black text-gray-900 dark:text-white transition-colors">{{ totalPresents }} / {{ chefs.length + jeunes.length }}</span>
                </div>
    ...
    <transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 translate-y-full" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition-all duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-full">
        ...
        <div v-if="showShoppingModal" class="fixed inset-0 z-50 flex flex-col justify-end printable-modal">
                ...
                <div class="relative bg-gray-50 dark:bg-gray-900 w-full h-[90%] rounded-t-3xl shadow-2xl flex flex-col z-10 overflow-hidden printable-content transition-colors">
                    
                    <div class="bg-[#004267] dark:bg-gray-800 text-white pt-5 pb-6 px-6 shrink-0 text-center rounded-t-3xl border-b-4 border-[#003B5C] dark:border-gray-700 transition-colors">
